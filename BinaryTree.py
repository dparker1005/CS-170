...
class BinaryTree:
    ...
    def getValueWithPath(self, path):
        ...
        #precondition(self.nodeExists(path))
        for n in self.nodes:
            if n.getPath() == path:
                return n.getValue()            
    ...
    def __repr__(self):
        ...
        while(len(nodesCopy)>0):
            ...
            if(node1.getPath()[-1]=='l'):
                ...
                toReturn=toReturn+'", "'
                toReturn=toReturn+"["+str(node1.getValue[0])+", "+str(node1.getValue[1])+"]\n"
                toReturn=toReturn+'", "'
                toReturn=toReturn+"["+str(node2.getValue[0])+", "+str(node2.getValue[1])+"]\n"
                toReturn=toReturn+'")'
            else:
                toReturn=toReturn+'\ntree = tree.withNewBranches("'
